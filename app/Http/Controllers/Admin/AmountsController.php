...
class AmountsController extends Controller
{
    ...
    public function index()
    {
    	...
    	return view('admin.amounts.index',$data);
    }
    ...
    public function accept_transfer($id = 0)
    {
        ...
        if(empty($result)){
            return redirect('admin/amounts')->with('error_message' , 'Invalid request to accept transfer.');
        }
        $accept_amount = $result->amount;
        $sender_cash = $this->cash_in_hand($result->from_user);
        if($sender_cash < $accept_amount){
            return redirect('admin/amounts')->with('error_message' , 'Sender has less amount in his account.');
        }
        ...
        return redirect('admin/amounts')->with('success_message' , 'Transfer has been accepted successfully.');
    }
    ...
    public function reject_transfer($id = 0)
    {
        ...
        $result = $amount->where('is_accepted' , 1)->where('user_id' , $user->id)->find($id);
        if(empty($result)){
            return redirect('admin/amounts')->with('error_message' , 'Invalid request to reject transfer.');
        }
        $amount->where('id',$id)->update(['is_accepted'=>3]);
        return redirect('admin/amounts')->with('success_message' , 'Transfer has been rejected successfully.');
    }
